...
(function(){
    var flower = document.getElementById('flower'),
        ...
        height = 640,
        angle = 0,
        step = (2*Math.PI) / numOfPetals;

    for (var i = 0; i < numOfPetals; i++) {

        var x = Math.round(width/2 + radius * Math.cos(angle) - 160/2),
            y = Math.round(height/2 + radius * Math.sin(angle) - 160/2),
            rotation = angle * 180 / Math.PI + 135;
        
        flower.innerHTML += petalTemplate.innerHTML;
...
(function() {
    ...
    function getPetalNum(xAxis, yAxis, ratio) {
        var petal = 0;
    ...
    window.getPetalNum = getPetalNum;
...
(function() {
    ...
    window.updateWheel = function(gamepad) {
      var petalNum = window.getPetalNum(gamepad.axes[0], gamepad.axes[1], (1/numOfPetals)*360/100);
      if (!petalNum) {
          return;
      }
      ...
      for (var i = 0; i < petals.length; i++) {
          petals[i].className = 'petal';
      }
      
      petals[petalNum - 1].className += ' selected';
    }
...
//     tester.updateGamepads(gamepadSupport.gamepads);
    ...
    gamepadSupport.startPolling();
    console.log('on gamepad connected')
  },
...
//         tester.updateGamepads(gamepadSupport.gamepads);
  ...
  gamepadEvent: function(gamepadId) {
      var gamepad = gamepadSupport.gamepads[gamepadId];
      
      window.updateWheel(gamepad);
    }
